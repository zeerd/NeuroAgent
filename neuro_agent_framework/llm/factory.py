...
class LLMFactory:
    ...
    @classmethod
    def create(cls, provider: str, config: LLMConfig, instance_id: Optional[str] = None) -> BaseLLM:
        """
        创建 LLM 实例
        ...
        if provider not in registry._providers:
            raise ValueError(f"Unknown provider: {provider}. Available: {registry.list_providers()}")
        ...
        if instance_id and instance_id in cls._instances:
            ...
            if cached.config.model == config.model and cached.config.api_base == config.api_base:
    ...
    @classmethod
    def create_from_config(cls, config: Dict[str, Any], instance_id: Optional[str] = None) -> BaseLLM:
        """
        从配置字典创建 LLM 实例
